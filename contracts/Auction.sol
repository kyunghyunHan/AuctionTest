...
contract Auction {
   ...
   constructor() payable {
       beneficiary = msg.sender; // modifier
 
       uint[] memory emptyArray;
       items[0] = Item({itemId:0,itemTokens:emptyArray}); // 인스턴스 컨트랙트 배포시 3개
       items[1] = Item({itemId:1,itemTokens:emptyArray});
       items[2] = Item({itemId:2,itemTokens:emptyArray});
   }
  
   function register() public payable {
       Person storage bidder = bidders[bidderCount];
       bidder.personId = bidderCount;
       bidder.addr = msg.sender;
       bidder.remainingTokens = 5;
